...
- built-in PyTorch reference backend with real decoder-only `zero-8m` and `zero-25m` birth, from-scratch pretraining, continued pretraining, full-parameter causal SFT, merged-output LoRA SFT, merged-output QLoRA SFT, Direct Preference Optimization (DPO), and teacher-to-smaller-student knowledge distillation for Frontierwright reference-model lineages;
...
- stable JSON/non-interactive machine surfaces for project/model/birth/evolve/optimize/operate/resource/build/stats/evaluation/data/path/plan/run/candidate/Lab-adapter operations;
